...
public class Main {
		...
		try (CloseableHttpClient closeableHttpClient =
			...
			if (statusLine.getStatusCode() == HttpStatus.SC_OK) {
				...
				if (responseJSONObject.has("error")) {
					throw new Exception(
						"Error while adding ddmTemplate: " +
	...
	private void _addOrUpdateDDMStructures(long groupId) throws Exception {
		System.out.println("Checking for ddmStructures");
		...
		for (Path path : paths) {
			System.out.println("Found ddmStructure " + path);
			...
			try {
				...
				System.out.println(
					"DataDefinition was not found " + exception.getMessage());

				System.out.println(
					"Adding new DataDefinition with key " + dataDefinitionKey);
