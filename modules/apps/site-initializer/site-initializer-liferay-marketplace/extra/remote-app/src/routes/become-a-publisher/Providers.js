...
import {getMarketplaceApplicationById} from './services/MarketplaceApplication';
...
const InitProvider = ({children}) => {
	...
	const getInitialData = async () => {
		...
		try {
			if (applicationId) {
				const {data} = await getMarketplaceApplicationById(applicationId);

				initialData = LiferayAdapt.adaptToMarketplaceApplicationToForm(
					data
				);
			}
		}
