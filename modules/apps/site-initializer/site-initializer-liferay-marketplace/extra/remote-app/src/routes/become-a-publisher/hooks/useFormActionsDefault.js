...
import {createOrUpdateMarketplaceApplication} from '../services/MarketplaceApplication';
...
const useFormActions = ({
	...
	const _onValidation = useCallback(() => {
		...
		if (phraseAgentPage) {
			...
			redirectTo(MARKETPLACE_PAGES.GET_IN_TOUCH);
			validated = false;
		}
		else {
	...
	const onSave = useCallback(
		async (status = {}) => {
			...
			try {
				const response = await createOrUpdateMarketplaceApplication(
					form,
					status
				);
	...
	const onNext = useCallback(async () => {
		...
		if (validated) {
			...
			redirectTo(MARKETPLACE_PAGES.HANG_TIGHT);
		}
