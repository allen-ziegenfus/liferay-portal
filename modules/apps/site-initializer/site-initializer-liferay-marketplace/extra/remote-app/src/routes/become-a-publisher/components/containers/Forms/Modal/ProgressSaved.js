...
import {MARKETPLACE_PAGES} from '../../../../../../common/utils/constants';
...
import {
	APPLICATION_STATUS,
	AVAILABLE_STEPS,
} from '../../../../../become-a-publisher/utils/constants';
import {AppContext} from '../../../../context/AppContextProvider';
import {updateMarketplaceApplicationStatus} from '../../../../services/MarketplaceApplication';
	...
	const onSendLinkAndExit = async () => {
		try {
			...
			await createQuoteRetrieve({
				...
				quoteRetrieveLink: `${origin}${liferaySiteName}/become-a-publisher?applicationId=${applicationId}`,
				retrieveEmail: email,
			});
			...
			await updateMarketplaceApplicationStatus(applicationId, status);

			clearExitAlert();

			redirectTo(MARKETPLACE_PAGES.HOME);
		}
		catch (error) {
