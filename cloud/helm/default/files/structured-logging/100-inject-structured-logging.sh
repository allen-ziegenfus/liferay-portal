...
then
	if [ -n "${LIFERAY_JVM_OPTS:-}" ]
	then
		export LIFERAY_JVM_OPTS="${LIFERAY_JVM_OPTS} -Djava.util.logging.config.file=${LOGGING_PROPERTIES_RUNTIME}"
	else
		export LIFERAY_JVM_OPTS="-Djava.util.logging.config.file=${LOGGING_PROPERTIES_RUNTIME}"
	fi
fi
